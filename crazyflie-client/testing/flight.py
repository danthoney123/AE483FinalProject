...
class CrazyflieClient:
    def __init__(self, uri, use_controller=False, use_observer=False, use_mocap=False, use_LED = True, use_safety=True, marker_deck_ids=None, set_bounds = False, bounds={}, filename='hardware_data'):
...
if __name__ == '__main__':
    ...
    flight_commands = [
        ...
        # # # hover
        # lambda: drone_client.move(0.0, 0.0, 0.5, 0.0, 4.0),
        ...
        # # # LED Effect Test
        # lambda: [drone_client.cf.param.set_value('ring.effect', number) or time.sleep(0.75) for number in range(19)],
...
#     start_time = time.time()
#     def test_callback(name, group):
#         print(name, group)
#         print(f'Callback at {time.time() - start_time }')

#     # mem = Memory(crazyflie=drone_client.cf)
    
#     memy = drone_client.cf.mem
#     # mem.refresh(refresh_done_callback=test_callback)
#     # print(memy.get_mem(id=4)) ### LED driver, 24 bytes 
#     # print(memy.get_mem(id=5)) ### LED Memory driver, 44 bytes

#     drone_client.cf.param.set_value('ring.effect', 13)

#     test_leds_driver = LEDLib.LEDDriverMemory(id=4, type='LED driver', size=24, mem_handler=memy)
# ########################## Basic Test ###############################

#     # for led in test_leds_driver.leds:
#     #     led.r = 255
#     #     led.g = 255
#     #     led.b = 255
#     #     led.intensity = 100
#     # print(f'4. LEDs done setting at {time.time() - start_time }')

#     # test_leds_driver.write_data(None, start_time=start_time)
#     # test_leds_driver.write_data(None, start_time=start_time)
...
#         for led in test_leds_driver.leds:
#             # print(f'Start at: {time.time()-start_time}')
#             led.r = random.randint(0, 255)
#             led.g = random.randint(0, 255)
#             led.b = random.randint(0, 255)
#             led.intensity = random.randint(0, 100)
#             # print(f'LEDs done setting at: {time.time()-start_time}')
#         test_leds_driver.write_data(None, start_time=start_time)
#         time.sleep(0.01667)
#         print(f'LEDs sent at: {time.time()-start_time}')


# ################################ Ring Timing Test #####################

#     led_list = test_leds_driver.leds
#     # while(time.time() - start_time < 15):
#     #     i_old = 11
#     #     for i in range(0, 12):
#     #         led_list[i].r = 255
...
#     #         test_leds_driver.write_data(None, start_time=start_time)
#     #         time.sleep(0.01667)
#     #         print(f'writing at {time.time() - start_time}')
