...
class CrazyflieClient:
    def __init__(self, uri, use_controller=False, use_observer=False, marker_deck_ids=None, set_bounds = False, bounds={}):
    ...
    def _fully_connected(self, uri):
        ...
        list_of_bounds = ["n_x_lower", "n_x_upper",
                          "n_y_lower", "n_y_upper",
                          "r_lower", "r_upper",
                          "psi_lower", "psi_upper",
                          ...
                          "v_y_lower", "v_y_upper",
                          "v_z_lower", "v_z_upper",
                          "p_x_int_lower", "p_x_int_upper",
                          "p_y_int_lower", "p_y_int_upper",
                          "p_z_int_lower", "p_z_int_upper",
                          "v_x_int_lower", "v_x_int_upper",
                          "v_y_int_lower", "v_y_int_upper",
                          "v_z_int_lower", "v_z_int_upper",
                          "a_x_in_W_lower", "a_x_in_W_upper",
                          "a_y_in_W_lower", "a_y_in_W_upper",
                          "a_z_in_W_lower", "a_z_in_W_upper"]
        ...
        if self.set_bounds:
            for key in self.bounds.keys():
                print(f'Setting new bound for {key} at index {list_of_bounds.index(key)}')
                self.cf.param.add_update_callback(group='ae483par', name='bounds_update', cb=param_callback)
                # self.cf.param.set_value('ae483par.bounds_update', list_of_bounds.index(key))
                # self.cf.param.set_value('ae483par.bounds_value', self.bounds[key])
                self.cf.param.set_value('ae483par.bounds_update', self.float_to_custom(list_of_bounds.index(key), self.bounds[key]))
...
if __name__ == '__main__':
    ...
    list_of_bounds = ["n_x_lower", "n_x_upper",
                        ...
                        "a_z_in_W_lower", "a_z_in_W_upper"]
    BOUNDS = {}
    for i, name in enumerate(list_of_bounds):
        BOUNDS[name] = i
    ...
    while(drone_client.params_sent < len(BOUNDS) and drone_client.set_bounds):
        time.sleep(0.1)
    print('All bounds recieved.')
    # Just stop, please, just stop
    # drone_client.stop(10.0)

    # Graceful takeoff
    drone_client.move(0.0, 0.0, 0.1, 0.0, 0.25)
    # drone_client.move_smooth([-3.417, 1.023, 0.085+0.2], [-3.417, 1.078, 0.085+0.5], 0.0, 0.20)
    # drone_client.move(-3.417, 1.023, 0.085+0.5, 0.0, 5.0)
    ...
    # Graceful landing
    # drone_client.move_smooth([0., 0., 0.50], [0., 0., 0.00], 0.0, 0.20)
