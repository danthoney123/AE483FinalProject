...
class CrazyflieClient:
    ...
    def initialize_offset(self):
        ...
        if self.use_mocap:
            ...
            self.offset = [mocap_x, mocap_y, mocap_z, 0.0] # Zero psi for now, drone must be start facing x+
...
if __name__ == '__main__':
    ...
    flight_commands = [
        ...
        # hover
        lambda: drone_client.move(0.0, 0.0, 0.5, 0.0, 5.0),

        # land
        lambda: drone_client.move_smooth([0.0, 0.0, 0.5], [0.0, 0.0, 0.0], 0.0, 0.2),

        # lambda: drone_client.move(0.0+offset[0], 0.0+offset[1], 0.2+offset[2], 0.0, 1.0),
        # lambda: drone_client.move_smooth([0.+offset[0], 0.+offset[1], 0.2+offset[2]], [0.+offset[0], 0.+offset[1], 0.5+offset[2]], 0.0, 0.20),
        # lambda: drone_client.move(0.0+offset[0], 0.0+offset[1], 0.5+offset[2], 0.0, 1.0),
        
        # # Move in a square (with a pause at each corner)
        # lambda: drone_client.move_smooth([0.0+offset[0], 0.0+offset[1], 0.5+offset[2]], [0.5+offset[0], 0.0+offset[1], 0.5+offset[2]], 0.0, 0.20),
        # lambda: drone_client.move(0.5+offset[0], 0.0+offset[1], 0.5+offset[2], 0.0, 1.0),
        # lambda: drone_client.move_smooth([0.5+offset[0], 0.0+offset[1], 0.5+offset[2]], [0.5+offset[0], 0.5+offset[1], 0.5+offset[2]], 0.0, 0.20),
        # lambda: drone_client.move(0.5+offset[0], 0.5+offset[1], 0.5+offset[2], 0.0, 1.0),
        # lambda: drone_client.move_smooth([0.5+offset[0], 0.5+offset[1], 0.5+offset[2]], [0.0+offset[0], 0.5+offset[1], 0.5+offset[2]], 0.0, 0.20),
        # lambda: drone_client.move(0.0+offset[0], 0.5+offset[1], 0.5+offset[2], 0.0, 1.0),
        # lambda: drone_client.move_smooth([0.0+offset[0], 0.5+offset[1], 0.5+offset[2]], [0.0+offset[0], 0.0+offset[1], 0.5+offset[2]], 0.0, 0.20),

        # # Graceful landing
        # lambda: drone_client.move(0.0+offset[0], 0.0+offset[1], 0.5+offset[2], 0.0, 1.0),
        # lambda: drone_client.move_smooth([0.+offset[0], 0.+offset[1], 0.50+offset[2]], [0.+offset[0], 0.+offset[1], 0.2+offset[2]], 0.0, 0.20),
        # lambda: drone_client.move(0.0+offset[0], 0.0+offset[1], 0.20+offset[2], 0.0, 1.0),
        # lambda: drone_client.move(0, 0, 0.2, 0, 0.5),
        # lambda: drone_client.move_smooth([0, 0, 0.2], [0, 0, 0.5], 0, 0.2),
        # lambda: drone_client.move(0, 0, 0.5, 0, 3),
        # lambda: drone_client.move_smooth([0, 0, 0.5], [0, 0, 0], 0, 0.2)
    ...
    if use_mocap:
        print(f"Mocap reports [{mocap_client.data['x'][-1]}, {mocap_client.data['y'][-1]}, {mocap_client.data['z'][-1]}]")
    print('==========================================================================')
