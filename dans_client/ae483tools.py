...
def transform_data_mocap(raw_data):
    ...
    R_inQ_ofA = Rotation.from_euler('zyx', [psi, theta, phi], degrees=False).as_matrix()           # <-- FIXME
    ...
    R_inW_ofQ = R_inQ_ofW.transpose()                  # <-- FIXME
    p_inW_ofQ = -R_inQ_ofW.transpose()@p_inQ_ofW      # <-- FIXME
    ...
    for i in range(len(data['time'])):
        ...
        angles = Rotation.from_matrix(R_inW_ofB).as_euler('zyx')
        data['yaw'][i], data['pitch'][i], data['roll'][i] = angles[0], angles[2], angles[1]    # <-- FIXME
        #
...
def getMinTimestep(raw_data_mocap, t, z_drone):
    # Create an array with a range of time shifts
    t_shifts = np.linspace(0., 1, 21)
...
def sync_data_mocap(raw_data_mocap, t, z_drone, transform=True):
    ...
    t_shift_min = getMinTimestep(raw_data_mocap, t, z_drone)    # <-- FIXME
