...
//Initial mocap positions
static float x_init = 0.0f;
static float y_init = 0.0f;
static float psi_init = 0.0f;
static bool initial_position_flag = true;
...
void ae483UpdateWithPose(poseMeasurement_t *meas)
{
  ...
  if (initial_position_flag) {
    x_init = p_x_mocap;
    y_init = p_y_mocap;
    psi_init = psi_mocap;
    initial_position_flag = false;
  }
}
                     ...
                     const stabilizerStep_t stabilizerStep)
{
  ...
  if (use_observer) {
    ...
    p_x += dt * (0.00466435442905244f * n_x - 2.77471878583012f * p_x +  2.77471878583012f * p_x_mocap + 0.961821739580306f * v_x) ;
    p_y += dt * (0.000205953270931819f * n_y - 6.63785538495217f * p_y +  6.63785538495217f * p_y_mocap +  0.99831424954267f * v_y);

    p_z += dt * (-2.43480856546917f * (p_z - p_z_eq)  + 2.43480856546916f * (r - p_z_eq) + v_z) ;

    psi += dt * (-0.285714285714286f * psi + 0.285714285714286f * psi_mocap + w_z );
...
LOG_ADD(LOG_FLOAT, x_init, &x_init)
LOG_ADD(LOG_FLOAT, y_init, &y_init)
LOG_ADD(LOG_FLOAT, psi_init, &psi_init)
