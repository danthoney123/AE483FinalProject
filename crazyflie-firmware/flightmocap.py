...
if __name__ == '__main__':
 ...
 #going up
    x_init = -2.4736363887786865
    y_init = -0.023464005440473557
    z_init = 0.08153686672449112
    # drone_client.move(x_init, y_init, z_init + 0.2, 0.0, 1.0)
    # drone_client.move(x_init, y_init, z_init + 0.5, 0.0, 10.0)
    # drone_client.move(x_init, y_init, z_init + 0.2, 0.0, 1.0)
...
#Land
    ...
    #Graceful takeoff
    # drone_client.move(x_init, y_init, z_init + 0.2, 0.0, 1.0)
    # drone_client.move_smooth([x_init, y_init, z_init + 0.2], [x_init + 0., y_init + 0., z_init + 0.5], 0.0, 0.20)
    # drone_client.move(x_init, y_init, z_init +0.5, 0.0, 0.5)
    
    # # Move in a square (with a pause at each corner)
    # num_squares = 5
    # for i in range(num_squares):
    #     drone_client.move_smooth([x_init + 0.0, y_init + 0.0, z_init + 0.5], [x_init +0.5, y_init + 0.0, z_init + 0.5], 0.0, 0.20)
    #     drone_client.move(x_init +0.5, y_init + 0.0, z_init + 0.5, 0.0, 1.0)
    #     drone_client.move_smooth([x_init + 0.5, y_init + 0.0, z_init + 0.5], [x_init + 0.5,y_init +  0.5, z_init + 0.5], 0.0, 0.20)
    #     drone_client.move(x_init +0.5, y_init + 0.5, z_init + 0.5, 0.0, 1.0)
    #     drone_client.move_smooth([x_init + 0.5, y_init + 0.5, z_init + 0.5], [x_init + 0.0, y_init + 0.5, z_init + 0.5], 0.0, 0.20)
    #     drone_client.move(x_init +0.0, y_init +  0.5, z_init + 0.5, 0.0, 1.0)
    #     drone_client.move_smooth([x_init + 0.0, y_init + 0.5, z_init + 0.5], [x_init + 0.0, y_init + 0.0, z_init + 0.5], 0.0, 0.20)
    #     drone_client.move(x_init +0.0, y_init + 0.0, z_init + 0.5, 0.0, 1.0)

    # # Graceful landing
    # drone_client.move(x_init, y_init, z_init + 0.5, 0.0, 0.5)
    # drone_client.move_smooth([x_init, y_init, z_init + 0.50], [x_init + 0., y_init + 0., z_init + 0.20], 0.0, 0.20)
    # drone_client.move(x_init, y_init, z_init + 0.20, 0.0, 1.0)
    ...
    with open('../crazyflie-client/FinalProject/test_flight_2.json', 'w') as outfile:
